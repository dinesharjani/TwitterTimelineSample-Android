...
/**
 * A very simple {@link ArrayAdapter Adapter} to handle the Timeline. This time there's no fancy inversion going on; and the thing is
 * that the Adapter doesn't truly have a hold on the data, but a copy of it. The data in this sample is held by the Loader;
 * the {@link ArrayAdapter Adapter} simply links to it in its private list in order to display it. That's why I made the case of
 * compositing the {@link ArrayAdapter Adapter} with the {@link com.dhsoftware.android.TwitterTimelineSample.loaders.TwitterTimelineLoader TwitterTimelineLoader} for
